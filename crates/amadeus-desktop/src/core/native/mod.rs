...
mod imp {
    ...
    fn initialize_native_stt(
        config: amadeus_client::stt::config::SttRuntimeConfig,
                    ...
                    if let Some(service) = svc_guard.as_ref() {
                            ...
                            AgentRuntimeConfig::load(Some(workspace_root), None)
                        {
                            if runtime_cfg.services.local_llm {
                                runtime_cfg.provider = amadeus_backend::config::LlmProvider::LlamaCpp;
                            }
    ...
    fn read_config_top_key(workspace_root: &Path, key: &str) -> Option<String> {
        let path = workspace_root.join(amadeus_backend::config::DEFAULT_CONFIG_PATH);
    ...
    pub extern "C" fn amadeus_native_provider_active_type_index() -> i32 {
        ...
        match amadeus_backend::config::LlmProvider::parse(provider_str)
            .unwrap_or(amadeus_backend::config::LlmProvider::OpenAiChat)
        {
            amadeus_backend::config::LlmProvider::Anthropic => 0,
            amadeus_backend::config::LlmProvider::OpenAiChat
            | amadeus_backend::config::LlmProvider::OpenAiResponses => {
            ...
            amadeus_backend::config::LlmProvider::Gemini => 2,
            amadeus_backend::config::LlmProvider::Ollama => 4,
            amadeus_backend::config::LlmProvider::LlamaCpp => 5,
        }
    }
    ...
    pub extern "C" fn amadeus_native_llm_thinking() -> i32 {
        ...
        {
            i32::from(amadeus_backend::llm::llama_cpp::is_thinking())
        }
