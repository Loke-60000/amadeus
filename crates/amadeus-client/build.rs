...
fn build_linux_native_cubism() {
    ...
    // amadeus-client lives at crates/amadeus-client — workspace root is two levels up.
    let workspace_root = manifest_dir.join("..").join("..").canonicalize()
        .unwrap_or_else(|_| manifest_dir.join("..").join(".."));

    let native_cpp_src = manifest_dir.join("src").join("cubism");
    println!(
        "cargo:rerun-if-changed={}",
        workspace_root.join(THIRD_PARTY_DIR_NAME).display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        workspace_root.join(LOCAL_RESOURCE_DIR_NAME).display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        workspace_root.join(CUBISM_SDK_DIR_NAME).display()
    );

    let cubism_paths = resolve_cubism_paths(&workspace_root, &manifest_dir);
...
#[cfg(target_os = "linux")]
fn resolve_cubism_paths(workspace_root: &Path, manifest_dir: &Path) -> CubismPaths {
    ...
    let framework_src = resolve_cubism_framework_src(workspace_root, manifest_dir);
    let core_root = resolve_cubism_core_root(workspace_root, manifest_dir, &framework_src);
    CubismPaths {
        framework_src,
        core_root,
    }
...
#[cfg(target_os = "linux")]
fn resolve_cubism_framework_src(workspace_root: &Path, manifest_dir: &Path) -> PathBuf {
    ...
    let legacy_dir = workspace_root.join(CUBISM_SDK_DIR_NAME).join("Framework").join("src");
...
#[cfg(target_os = "linux")]
fn resolve_cubism_core_root(workspace_root: &Path, manifest_dir: &Path, _adjacent_hint: &Path) -> PathBuf {
    ...
    let legacy_dir = workspace_root.join(CUBISM_SDK_DIR_NAME).join("Core");
